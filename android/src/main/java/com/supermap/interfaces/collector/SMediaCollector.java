...
//        }
        if (!SMap.safeGetType(recordset.getDataset().getPrjCoordSys(), PrjCoordSysType.PCS_EARTH_LONGITUDE_LATITUDE)) {
            Point2Ds points = new Point2Ds();
            points.add(pt);
            PrjCoordSys desPrjCoorSys = new PrjCoordSys();
            desPrjCoorSys.setType(PrjCoordSysType.PCS_EARTH_LONGITUDE_LATITUDE);
            CoordSysTranslator.convert(points, recordset.getDataset().getPrjCoordSys(), desPrjCoorSys,
        ...
        String[] pathArr = mediaFilePaths != null && !mediaFilePaths.equals("") ? mediaFilePaths.split(",") : new String[0];
...
//                                }
    ...
    /**
     * 把AI识别的结果添加到数据集中(图片直接保存在Media文件夹中)
     * @param info
     * @param addToMap
     * @param promise
     */
    @ReactMethod
    public void addArMedia(ReadableMap info, boolean addToMap, Promise promise) {
        try {
            boolean result = false;
            SMMediaCollector collector = SMMediaCollector.getInstance();
            ...
            if (collector == null || collector.getMediaPath().equals("")) {
                ...
                if (media.setMediaDataset(ds, datasetName)) {
                    if (addToMap) {
                        ...
                        result = media.saveArMedia(mediaName, collector.getMediaPath(), true);
...
//                    }
            ...
            if (media.getFileName() == null) {
                ...
                WritableMap dic = Arguments.createMap();
                dic.putString("name", "MediaName");
                dic.putString("value", fileName);
                infos.pushMap(dic);
            }
